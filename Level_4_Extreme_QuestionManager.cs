    ...
    public void nextButtonPressed()
    {
        ...
        if (currentStatus == gameStatus.InCheck)
        {
            ...
            else
            {
                ...
                if (!PlayerPrefs.HasKey("Level_4_Modes"))
                {
                    PlayerPrefs.SetString("Level_4_Modes", "e");
                }
                else
                {
                    string modes = PlayerPrefs.GetString("Level_4_Modes");
                    modes += "e,";
                    PlayerPrefs.SetString("Level_4_Modes", modes);
                }

                if (!PlayerPrefs.HasKey("Level_4_High_Percentage"))
                {
                    PlayerPrefs.SetString("Level_4_Percentages", targetTime.ToString() + ",");
                }
                else
                {
                    string tscores = PlayerPrefs.GetString("Level_4_Percentages");

                    tscores += targetTime.ToString() + ",";
                    PlayerPrefs.SetString("Level_4_Percentages", tscores);
                }

                float timeP = Mathf.Round(Time.timeSinceLevelLoad);
                string sttime = timeP.ToString() + ",";
                Debug.Log("t");
                Debug.Log(sttime);

                PlayerPrefs.SetFloat("Time_P", timeP);

                if (!PlayerPrefs.HasKey("Level_4_Time"))
                {
                    PlayerPrefs.SetFloat("Level_4_Time", timeP);
                }


                if (!PlayerPrefs.HasKey("Level_4_Times"))
                {
                    PlayerPrefs.SetString("Level_4_Times", sttime);
                }
                else
                {
                    string tsttime = PlayerPrefs.GetString("Level_4_Times");
                    tsttime += sttime;
                    PlayerPrefs.SetString("Level_4_Times", tsttime);
                }
